    ...
    conn.on("message", (data) => {
        ...
        if (info) {
            switch (info.command) {
                ...
                case 'audioActive': {
                    const { active, roomId } = info
                    const caller_conn = rooms[roomId].find(client => client.conn !== conn)?.conn
                    if (caller_conn) send(caller_conn, { event: 'audioActive', active })
                }
            }
        } else {
    ...
    conn.on("close", () => {
        Object.entries(rooms).forEach(([room, clients]) => {
            ...
            if (rooms[room].length === 0) {
                delete rooms[room]
                if (temporarySDP[room]) delete temporarySDP[room]
                if (temporaryICE[room]) delete temporaryICE[room]
            }
        })
    })
